...
The command queue is used for driver commands and device responses to these
commands. The device MUST return the descriptor chains it receives as soon as
possible, and must never hold to them for indefinite periods of time.

The event queue is used for events sent by the device to the driver. The
driver MUST re-queue the descriptor chains returned by the device as soon as
possible, and must never hold on them for indefinite periods of time.
...
\item[\field{device_type}] (driver-read-only) informs the driver of which type
of video device it is dealing with. Corresponds with the \field{vfl_devnode_type}
field of the device.
...
\begin{enumerate}
\item The driver reads the \field{device_type} and \field{device_type} fields
from the configuration layout.
...
\devicenormative{\subparagraph}{Device Operation: Open device}{Device Types / Sound Device / Device Operation / Open device}
...
\devicenormative{\subparagraph}{Device Operation: Close device}{Device Types / Sound Device / Device Operation / Close device}
...
\item \textbf{_IORW} is writable by both the device and driver,
...
\begin{itemize}
\item In case of success, the device MUST always write the payload in the
device-writable part of the descriptor chain.
\item In case of failure, the device is free to write the payload in the
device-writable part of the descriptor chain or not. Some errors may still
result in the payload being updated, and in this case the device is expected
to write the updated payload. If the device has not written the payload after
an error, the driver MUST assume that the payload has not been modified.
\end{itemize}
...
\devicenormative{\subparagraph}{Device Operation: V4L2 ioctls}{Device Types / Sound Device / Device Operation / V4L2 ioctls}
...
\devicenormative{\subparagraph}{Device Operation: Unmapping a MMAP buffer}{Device Types / Sound Device / Device Operation / Unmapping a MMAP buffer}
